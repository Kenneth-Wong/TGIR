...
train_pipeline = [
    ...
    dict(type='Collect', keys=['img', 'gt_bboxes', 'gt_labels', 'gt_rels', 'gt_relmaps', 'gt_keyrels']),
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
...
model = dict(
    ...
    relation_head=dict(
        ...
        relation_sampler=dict(
            ...
            key_sample=True,     ### Do not forget this one !!!!!!!!!!!!! You need the key sample
        ),
        # relation ranker
        relation_ranker=dict(
            ...
            #strong: CEloss
            loss=dict(
                type='CrossEntropyLoss', use_sigmoid=False, ignore_index=-1, loss_weight=1.0),
            # loss=dict(
            #     type='SigmoidDRLoss', loss_weight=1.0),
            input_dim=4096),
        loss_object=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0),
        loss_relation=dict(
...
evaluation = dict(interval=1, metric='predcls', relation_mode=True, classwise=True, key_first=True)
# optimizer
...
log_level = 'INFO'
work_dir = './new_experiments/VGKR_PredCls_heth_area_gnn8head_sSCE_transformerranker_faster_rcnn_x101_64x4d_fpn_1x'
