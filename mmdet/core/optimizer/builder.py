...
def build_optimizer(model, optimizer_cfg):
    ...
    # if needed, freeze some modules
    freeze_modules = optimizer_cfg.pop('freeze_modules', None)
    if freeze_modules is not None:
        for module_name in freeze_modules:
            for name, p in model.named_parameters():
                if name.startswith(module_name):
                    p.requires_grad = False
    # The frozen modules maybe much more than the required grad modules.
    # So usually, we may firstly freeze then unfreeze several modules.
    required_grad_modules = optimizer_cfg.pop('required_grad_modules', None)
    if required_grad_modules is not None:
        for module_name in required_grad_modules:
            for name, p in model.named_parameters():
                if name.startswith(module_name):
