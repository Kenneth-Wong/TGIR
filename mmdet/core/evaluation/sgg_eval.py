...
class SGRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        self.nogc_result_dict[mode + '_recall'] = {ngc: {20: [], 50: [], 100: []} for ngc in self.nogc_thres_num}
        if mode == 'sgdet':
            self.result_dict['phrdet_recall'] = {20: [], 50: [], 100: []}
            self.nogc_result_dict['phrdet_recall'] = {ngc: {20: [], 50: [], 100: []} for ngc in self.nogc_thres_num}

    def _calculate_single(self, target_dict, prediction_to_gt, gt_rels, mode, nogc_num=None):
        target = target_dict[mode + '_recall'] if nogc_num is None else target_dict[mode + '_recall'][nogc_num]
        for k in target:
    ...
    def generate_print_string(self, mode):
        ...
        for nogc_num in self.nogc_thres_num:
            result_str += self._print_single(self.nogc_result_dict, mode, nogc_num)
            if mode == 'sgdet':
                result_str += self._print_single(self.nogc_result_dict, 'phrdet', nogc_num)
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        nogc_thres_num = self.nogc_thres_num  # list: (typlically) VG: [50]; VRD: [10, 70]
        ...
        if mode == 'sgdet':
            ...
            self._calculate_single(self.result_dict, pred_to_gt, gt_rels, mode='phrdet')
        ...
        for nogc_num in nogc_thres_num:
            ...
            if mode == 'sgdet':
                pred_to_gt = _compute_pred_matches(
                    ...
                    iou_thrs,
                    phrdet=True)
                local_container['phrdet_nogc@%d_pred_to_gt' % nogc_num] = pred_to_gt[:100]
                local_container['phrdet_nogc@%d_all_pred_to_gt' % nogc_num] = pred_to_gt  # for Pair accuracy
                self._calculate_single(self.nogc_result_dict, pred_to_gt[:100], gt_rels, mode='phrdet', nogc_num=nogc_num)
...
class SGZeroShotRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        self.nogc_result_dict[mode + '_zeroshot_recall'] = {ngc: {20: [], 50: [], 100: []} for ngc in
                                                            self.nogc_thres_num}
        if mode == 'sgdet':
            self.result_dict['phrdet_zeroshot_recall'] = {20: [], 50: [], 100: []}
            self.nogc_result_dict['phrdet_zeroshot_recall'] = {ngc: {20: [], 50: [], 100: []} for
                                                               ngc in self.nogc_thres_num}

    def _calculate_single(self, target_dict, prediction_to_gt, mode, nogc_num=None):
    ...
    def _print_single(self, target_dict, mode, nogc_num=None):
        ...
        suffix_type = 'Zero Shot Recall.' if nogc_num is None else 'NoGraphConstraint @ %d Zero Shot Recall.' % (
            nogc_num)
        result_str += ' for mode=%s, type=%s' % (mode, suffix_type)
        result_str += '\n'
        return result_str
    ...
    def calculate_recall(self, global_container, local_container, mode):
        ...
        if mode == 'sgdet':
            ...
            self._calculate_single(self.result_dict, phrdet_pred_to_gt, 'phrdet')

        # nogc
        for nogc_num in self.nogc_thres_num:
            nogc_pred_to_gt = local_container['nogc@%d_pred_to_gt' % nogc_num]
            self._calculate_single(self.nogc_result_dict, nogc_pred_to_gt, mode, nogc_num)

            if mode == 'sgdet':
                nogc_pred_to_gt = local_container['phrdet_nogc@%d_pred_to_gt' % nogc_num]
                self._calculate_single(self.nogc_result_dict, nogc_pred_to_gt, 'phrdet', nogc_num)
...
class SGPairAccuracy(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        self.nogc_result_dict[mode + '_accuracy_hit'] = {ngc: {20: [], 50: [], 100: []} for ngc in self.nogc_thres_num}
        self.result_dict[mode + '_accuracy_count'] = {20: [], 50: [], 100: []}
        self.nogc_result_dict[mode + '_accuracy_count'] = {ngc: {20: [], 50: [], 100: []} for ngc in
                                                           self.nogc_thres_num}

    def _calculate_single(self, target_dict, prediction_to_gt, gt_rels, mode, pred_pair_in_gt, nogc_num=None):
    ...
    def _print_single(self, target_dict, mode, nogc_num=None):
        ...
        suffix_type = 'TopK Accuracy.' if nogc_num is None else 'NoGraphConstraint @ %d TopK Accuracy.' % (nogc_num)
        result_str += ' for mode=%s, type=%s' % (mode, suffix_type)
        result_str += '\n'
        return result_str
    ...
    def prepare_gtpair(self, local_container):
        pred_pair_idx = local_container['pred_rel_inds'][:, 0] * 10000 + local_container['pred_rel_inds'][:, 1]
        gt_pair_idx = local_container['gt_rels'][:, 0] * 10000 + local_container['gt_rels'][:, 1]
    ...
    def calculate_recall(self, global_container, local_container, mode):
        if mode != 'sgdet':
            ...
            self._calculate_single(self.result_dict, pred_to_gt, gt_rels, mode, self.pred_pair_in_gt)

            # nogc
            for nogc_num in self.nogc_thres_num:

                nogc_pred_to_gt = local_container['nogc@%d_all_pred_to_gt' % nogc_num]
                self._calculate_single(self.nogc_result_dict, nogc_pred_to_gt, gt_rels, mode,
                                       local_container['nogc@%d_pred_pair_in_gt' % nogc_num], nogc_num)
...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def register_container(self, mode):
        ...
        self.result_dict[mode + '_mean_recall_collect'] = {20: [[] for _ in range(self.num_rel)],
                                                           50: [[] for _ in range(self.num_rel)],
                                                           100: [[] for _ in range(self.num_rel)]}
        self.result_dict[mode + '_mean_recall_list'] = {20: [], 50: [], 100: []}

        self.nogc_result_dict[mode + '_mean_recall'] = {ngc: {20: 0.0, 50: 0.0, 100: 0.0} for ngc in
                                                        self.nogc_thres_num}
        self.nogc_result_dict[mode + '_mean_recall_collect'] = {ngc: {20: [[] for _ in range(self.num_rel)],
                                                                      50: [[] for _ in range(self.num_rel)],
                                                                      100: [[] for _ in range(self.num_rel)]}
                                                                for ngc in self.nogc_thres_num}
        self.nogc_result_dict[mode + '_mean_recall_list'] = {ngc: {20: [], 50: [], 100: []} for ngc in
                                                             self.nogc_thres_num}

        if mode == 'sgdet':
            self.result_dict['phrdet_mean_recall'] = {20: 0.0, 50: 0.0, 100: 0.0}
            self.result_dict['phrdet_mean_recall_collect'] = {20: [[] for _ in range(self.num_rel)],
                                                              50: [[] for _ in range(self.num_rel)],
                                                              100: [[] for _ in range(self.num_rel)]}
            self.result_dict['phrdet_mean_recall_list'] = {20: [], 50: [], 100: []}

            self.nogc_result_dict['phrdet_mean_recall'] = {ngc: {20: 0.0, 50: 0.0, 100: 0.0} for ngc in
                                                           self.nogc_thres_num}
            self.nogc_result_dict['phrdet_mean_recall_collect'] = {ngc: {20: [[] for _ in range(self.num_rel)],
                                                                         50: [[] for _ in range(self.num_rel)],
                                                                         100: [[] for _ in range(self.num_rel)]}
                                                                   for ngc in self.nogc_thres_num}
            self.nogc_result_dict['phrdet_mean_recall_list'] = {ngc: {20: [], 50: [], 100: []} for ngc in
                                                                self.nogc_thres_num}
    ...
    def generate_print_string(self, mode, predicate_freq=None):
        ...
        if mode == 'sgdet':
            result_str += self._print_single(self.result_dict, 'phrdet', predicate_freq=predicate_freq)

        # nogc
        for nogc_num in self.nogc_thres_num:
            result_str += self._print_single(self.nogc_result_dict, mode, nogc_num, predicate_freq=predicate_freq)
            if mode == 'sgdet':
                result_str += self._print_single(self.nogc_result_dict, 'phrdet', nogc_num, predicate_freq=predicate_freq)
        return result_str
    ...
    def collect_mean_recall_items(self, global_container, local_container, mode):
        ...
        if mode == 'sgdet':
            phrdet_pred_to_gt = local_container['phrdet_pred_to_gt']
            self._collect_single(self.result_dict, phrdet_pred_to_gt, gt_rels, 'phrdet')
        ...
        for nogc_num in self.nogc_thres_num:
            ...
            if mode == 'sgdet':
                nogc_pred_to_gt = local_container['phrdet_nogc@%d_pred_to_gt' % nogc_num]
                self._collect_single(self.nogc_result_dict, nogc_pred_to_gt, gt_rels, 'phrdet', nogc_num)
    ...
    def calculate_mean_recall(self, mode):
        ...
        for nogc_num in self.nogc_thres_num:
            ...
            if mode == 'sgdet':
                self._calculate_single(self.nogc_result_dict, 'phrdet', nogc_num)
