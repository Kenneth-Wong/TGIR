...
def vg_evaluation(
    ...
    for m in modes:
        ...
        single_result_dict = vg_evaluation_single(m,
                                                  groundtruths,
                                                  predictions,
                                                  iou_thrs,
                                                  logger,
...
def vg_evaluation_single(
    ...
    # temporarily use the num_predicates to judge what is the type of the dataset
    if num_predicates == 51:
        dir_name = 'vg_evaluation'
    elif num_predicates == 71:
        dir_name = 'vrd_evaluation'
    elif num_predicates == 81:
        dir_name = 'visualgenomekr_evaluation'
    else:
        raise NotImplementedError
    zeroshot_triplet = torch.load("data/{}/zeroshot_triplet.pytorch".format(dir_name),
    ...
    eval_zeroshot_recall = SGZeroShotRecall(result_dict, nogc_result_dict, nogc_thres_num)
    ...
    eval_pair_accuracy = SGPairAccuracy(result_dict, nogc_result_dict, nogc_thres_num)
...
def format_result_dict(result_dict, result_str, mode):
    ...
    for k, v in result_dict[mode + '_recall'].items():
        ...
        copy_stat_str += (mode + '_recall_' + 'R_%d: ' % k + '{:0.3f}'.format(np.mean(v)) + '\n')
    # zeroshot recall
    for k, v in result_dict[mode + '_zeroshot_recall'].items():
        value = -1 if len(v) == 0 else np.mean(v)
        formatted[mode + '_zeroshot_recall_' + 'zR_%d' % k] = value
        copy_stat_str += (mode + '_zeroshot_recall_' + 'zR_%d: ' % k + '{:0.3f}'.format(value) + '\n')
    # mean recall
    ...
    if mode != 'sgdet':
        ...
        for k, v in result_dict[mode + '_accuracy_hit'].items():
            ...
            copy_stat_str += (mode + '_accuracy_hit_' + 'A_%d: ' % k + '{:0.3f}'.format(a_hit/a_count) + '\n')
