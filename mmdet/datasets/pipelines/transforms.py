...
class Resize(object):
    ...
    def _resize_map(self, results):
        for key in results.get('map_fields', []):
            if self.keep_ratio:
                gt_map = mmcv.imrescale(
                    results[key], results['scale'], interpolation='nearest')
            else:
                gt_map = mmcv.imresize(
                    results[key], results['scale'], interpolation='nearest')
            results[key] = gt_map
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        if self.flip_ratio is None: # test phase, must be 100% True or False
            assert 'flip' in results
        else: # training phase, flip with a certain probability, if flip_ratio is 0: close the flip gate
...
@PIPELINES.register_module
class MapRescale(object):
    """Rescale maps (e.g., saliency maps).
...
class MapNormalize(object):
    ...
    Args:
        mean (sequence): Mean values of 1 channels.
        std (sequence): Std values of 1 channels.
    """
    ...
    def __call__(self, results):
        for key in results.get('map_fields', []):
            results[key] = mmcv.imnormalize(results[key], self.mean, self.std, False)
        return results
