...
from .approaches import DirectionAwareMessagePassing, DirectionAwareMessagePassingPTS, \
    get_pattern_labels, get_internal_labels, top_down_induce
...
class VRPHeadV2(RelationHead):
    def __init__(self, **kwargs):
        super(VRPHeadV2, self).__init__(**kwargs)
    ...
    def build_structure(self):
        ...
        # The modules contains trainable parameters
        self.point_context_layer = DirectionAwareMessagePassingPTS(self.head_config, self.obj_classes)
    ...
    def build_composition(self):# setup the scene prediction branch
        if self.use_scene:
    ...
    def forward(self,
        ...
        if self.composition_augment and not is_testing:
            ...
            if self.training and self.use_visual_sim:
            ...
            if len(positive_inds) > 0:
                ...
                # 7. Combine the composition triplets for further relation scores inference
        ...
        # OPTIONAL: pattern cls
        if not self.traditional_rel_cls:
            roi_context_feats_point = self.point_context_layer(roi_feats_point, union_feats_point, det_result)
            split_roi_context_feats_point = roi_context_feats_point.split(num_objs)
            pair_reps_point = []
            for pair_idx, obj_rep_point in zip(det_result.rel_pair_idxes, split_roi_context_feats_point):
