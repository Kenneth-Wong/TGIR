...
from .. import builder
from mmdet.models.relation_heads.approaches.het_util import generate_forest
from .approaches import HybridLSTMContext
...
class HETHead(RelationHead):
    def __init__(self, **kwargs):
        super(HETHead, self).__init__(**kwargs)
        self.context_layer = HybridLSTMContext(self.head_config, self.obj_classes, self.rel_classes, self.statistics)
        ...
        # for het construction
        self.pick_parent = self.head_config.pick_parent
        self.isc_thresh = self.head_config.isc_thresh
        self.child_order = self.head_config.child_order

        # layer embedding
        self.num_embed_depth = None
        self.depth_embeddings = None
        self.depth_embed_dim = None
        self.embed_cls = None
        self.embed_tree_depth = getattr(self.head_config, 'embed_tree_depth', False)
        if self.embed_tree_depth:
            self.num_embed_depth = getattr(self.head_config, 'num_embed_depth', 4)
            self.depth_embed_dim = getattr(self.head_config, 'depth_embed_dim', self.context_pooling_dim)
            self.depth_embeddings = nn.Embedding(self.num_embed_depth, self.depth_embed_dim)
            self.embed_cls = nn.Sequential(*[nn.Linear(self.depth_embed_dim, self.depth_embed_dim),
                                             nn.BatchNorm1d(self.depth_embed_dim),
                                             nn.ReLU(inplace=False),
                                             nn.Dropout(0.5),
                                             nn.Linear(self.depth_embed_dim, self.depth_embed_dim),
                                             nn.BatchNorm1d(self.depth_embed_dim),
                                             nn.ReLU(inplace=False),
                                             nn.Dropout(0.5),
                                             nn.Linear(self.depth_embed_dim, self.num_embed_depth)])
            loss_embed_tree = getattr(self.head_config, 'loss_embed_tree',
                                      dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0))
            self.loss_embed_tree = builder.build_loss(loss_embed_tree)
    ...
    def forward(self,
        ...
        forest, depth_labels = generate_forest(det_result, pick_parent=self.pick_parent,
                                               isc_thresh=self.isc_thresh, child_order=self.child_order,
                                               num_embed_depth=self.num_embed_depth, need_depth=self.embed_tree_depth)
        if self.embed_tree_depth:
            roi_depth_embeddings = self.depth_embeddings(depth_labels)
            roi_feats = roi_feats + roi_depth_embeddings.detach()

        refine_obj_scores, obj_preds, edge_ctx = self.context_layer(roi_feats, forest, det_result)
        ...
        if self.training:
            ...
            # compute the head_spec_loss
            if self.embed_tree_depth:
                #depth_labels = torch.cat(depth_labels, 0).long().to(roi_feats.device)
                embed_scores = self.embed_cls(self.depth_embeddings(torch.range(0, self.num_embed_depth-1).long().to(rel_scores.device)))
                embed_loss = self.loss_embed_tree(embed_scores, torch.range(0, self.num_embed_depth-1).long().to(embed_scores.device))
                det_result.head_spec_losses = dict(loss_embed_tree=embed_loss)
