...
from .vctree_util import generate_forest, arbForest_to_biForest, get_overlap_info
from .treelstm_util import TreeLSTM_IO, MultiLayer_BTreeLSTM, BiTreeLSTM_Backward, BiTreeLSTM_Foreward
...
class VCTreeLSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, statistics):
        ...
        # VCTree--modified
        #prior_fg_matrix = statistics['fg_matrix'].float().sum(-1)
        #prior_likelihood = torch.log(prior_fg_matrix + 1e-3)
        #self.bi_freq_prior = nn.Linear(self.num_obj_classes * self.num_obj_classes, 1, bias=False)
        #with torch.no_grad():
        #    prior_likelihood = prior_likelihood + prior_likelihood.transpose(0, 1)
        #    self.bi_freq_prior.weight.copy_(prior_likelihood.view(-1).unsqueeze(0), non_blocking=True)
    ...
    def obj_ctx(self, num_objs, obj_feats, obj_labels=None, vc_forest=None, ctx_average=False):
        """
        ...
        for i, (feat, tree) in enumerate(zip(obj_feats, vc_forest)):
            ...
            if self.mode != 'predcls':
                ...
                obj_dist, obj_pred = self.decoder_rnn(tree, decoder_inp, num_objs[i])
            else:
    ...
    def forward(self, x, det_result, all_average=False, ctx_average=False):
        ...
        # 128 + 128 + 128 + 128 = 512
        bi_inp = torch.cat((self.obj_reduce(x.detach()), self.emb_reduce(obj_embed.detach()), box_inp, pair_inp), -1)
        bi_preds, vc_scores = self.vctree_score_net(num_objs, bi_inp, obj_dists)  # list of N x N
        forest = generate_forest(vc_scores, det_result)
        vc_forest = arbForest_to_biForest(forest)

        # object level contextual feature
        obj_ctxs, obj_preds, obj_dists = self.obj_ctx(num_objs, obj_pre_rep, obj_labels, vc_forest,
                                                      ctx_average=ctx_average)
        # edge level contextual feature
        obj_embed2 = self.obj_embed2(obj_preds.long())

        if (all_average or ctx_average) and self.effect_analysis and (not self.training):
            obj_rel_rep = torch.cat((self.untreated_edg_feat.view(1, -1).expand(batch_size, -1), obj_ctxs), dim=-1)
        else:
    ...
    def vctree_score_net(self, num_objs, roi_feat, roi_dist):
        ...
        for sub, obj, dist in zip(sub_feats, obj_feats, roi_dists):
            ...
            num_obj = sub.shape[0]
            num_dim = sub.shape[-1]
            sub = sub.view(1, num_obj, num_dim).expand(num_obj, num_obj, num_dim)  # N, N, 512
            obj = obj.view(num_obj, 1, num_dim).expand(num_obj, num_obj, num_dim)  # N, N, 512
            sub_dist = dist.view(1, num_obj, -1).expand(num_obj, num_obj, -1).unsqueeze(2)  # N, N, 1, 151
            obj_dist = dist.view(num_obj, 1, -1).expand(num_obj, num_obj, -1).unsqueeze(3)  # N, N, 151, 1
            joint_dist = (sub_dist * obj_dist).view(num_obj, num_obj, -1)  # N, N, (151, 151)

            #co_prior = self.bi_freq_prior(joint_dist.view(num_obj * num_obj, -1)).view(num_obj, num_obj)  # N, N
            vis_prior = self.vision_prior(torch.cat([sub * obj,
                                                     sub,
                                                     obj],
                                                     #co_prior.unsqueeze(-1)],
                                                    dim=-1).view(num_obj * num_obj, -1)).view(num_obj, num_obj)
