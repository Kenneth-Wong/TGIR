...
from mmdet.core import multiclass_nms_for_cluster
...
class Result(object):
    ...
    def __init__(self,
                 ...
                 formatted_masks=None,    # OD: Transfrom the masks for object detection evaluation
                 ...
                 formatted_bboxes=None,   # OD: Transfrom the refine_bboxes for object detection evaluation
...
class PostProcessor(nn.Module):
    ...
    def forward(self, det_result, key_first=False):
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, bbox) in enumerate(zip(
                relation_logits, finetune_obj_logits, rel_pair_idxes, det_result.bboxes)):
...
class DemoPostProcessor(object):
    """
    This API is used for obtaining the final information for demonstrating the scene graphs.
    It's usually invoked after the PostProcessor.
    Especially applying NMS to suppress the repetition.
    """
    def __init__(self):
    ...
    def filter_AB_rels(self, det_result):
        ...
        if len(keep_rel_idxes) > 0:
            det_result.rels = np.hstack((det_result.rel_pair_idxes, det_result.rel_labels[:, None]))
        else:
    ...
    def filter_nonoverlap_rels(self, det_result, must_overlap_predicates=None):
        ...
        keep_rel_idxes = []
        for idx, (pair, predicate) in enumerate(zip(rel_pair_idxes, rel_labels)):
            subj, obj = pair[0], pair[1]
            iou = ious[subj, obj]
    ...
    def filter_duplicate_triplets(self, det_result, vocab_objects, vocab_predicates):
        all_triplets = []
        new_rel_pair_idxes = []
        refine_labels = det_result.refine_labels
        rel_pair_idxes = det_result.rel_pair_idxes
        ...
        keep_rel_idxes = []
        for idx, (pair, predicate) in enumerate(zip(rel_pair_idxes, rel_labels)):
            triplet = [vocab_objects[refine_labels[pair[0]]], vocab_predicates[predicate],
    ...
    def forward(self, det_result, vocab_objects, vocab_predicates, object_thres=0.01, nms_thres=0.1, ignore_classes=None,
                must_overlap_predicates=None, max_rel_num=None):
        ...
        keep_obj_ids = np.where(np.isin(det_result.refine_labels, ignore_classes) == 0)[0]
        ...
        # apply NMS
        nms_keep_obj_ids, gathered = multiclass_nms_for_cluster(det_result.refine_bboxes[:, :-1],
                                                                det_result.refine_bboxes[:, -1],
                                                                det_result.refine_labels, nms_thres=nms_thres)
        det_result = self.clean_relations_via_objects(nms_keep_obj_ids, det_result)
        if len(nms_keep_obj_ids) == 0:
            return det_result

        # NOTE: This may be not necessary: Suppress the low-score objects
        score_keep_obj_ids = np.where(det_result.refine_bboxes[:, -1] >= object_thres)[0]
        det_result = self.clean_relations_via_objects(score_keep_obj_ids, det_result)
        ...
        # Filter the rels whose pairs must be overlaped
        det_result = self.filter_nonoverlap_rels(det_result, must_overlap_predicates)

        # Filter the duplicate triplets
        det_result = self.filter_duplicate_triplets(det_result, vocab_objects, vocab_predicates)
        ...
        rel_num = max_rel_num if max_rel_num is not None else int(num_obj * (num_obj - 1) / 2 - num_obj)
...
def group_regions(result, prior_pairs, thres=0.9):
    ...
    for boxes, labels in zip(bboxes, obj_labels):
        ...
        if num_obj == 0:
            ...
            continue
        box_areas = (boxes_np[:, 2] - boxes_np[:, 0] + 1) * (boxes_np[:, 3] - boxes_np[:, 1] + 1)
                    ...
                    if subj_area > obj_area:
                        if intersect[j, i] > thres:
                            if j in region_group:
                                region_group[i] = list(set(region_group[i] + region_group[j] + [j]))
                            else:
                                region_group[i].append(j)
                    else:
                        if intersect[i, j] > thres:
                            if i in region_group:
                                region_group[j] = list(set(region_group[j] + region_group[i] + [i]))
...
def get_internal_labels(leaf_labels, hierarchy, vocab):
    ...
    for idx, leaf_label in enumerate(leaf_labels_np):
        ...
        start_node = anytree.search.find(hierarchy, lambda node: node.id == leaf_name)
        iter_node = start_node
        while iter_node.parent is not None:
            iter_node = iter_node.parent
            internal_labels[idx].append(vocab.index(iter_node.id))
        internal_labels[idx] = torch.from_numpy(internal_labels[idx]).to(leaf_labels)
...
def _topdown_hook(root, output_vector, input_vector, vocab, reduce='avg'):
    if len(root.children) == 0:
        if root.id == 'Root':
            ...
            output_vector[vocab.index(root.id)] = input_vector[vocab.index(root.id)]
            return output_vector
    else:
        gather_values = []
        for c in root.children:
            output_vector = _topdown_hook(c, output_vector, input_vector, vocab, reduce)
...
def top_down_induce(x, hierarchy, vocab, reduce='avg', solveroot=None):
    ...
    if solveroot is not None:
        ...
        vocab_vec = _topdown_hook(hierarchy, vocab_vec, x, vocab, reduce=reduce)
    vocab_vec += 1e-7
    return vocab_vec
