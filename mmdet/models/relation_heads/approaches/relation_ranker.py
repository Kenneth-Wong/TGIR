...
class LSTMRanker(nn.Module):
    def __init__(self, input_dim=1024, hidden_dim=512, dropout_rate=0.2, nl_layer=1, bidirectional=True, num_out=1):
        ...
        self.num_out = num_out
        self.ranking_ctx_rnn = torch.nn.LSTM(
    ...
    def forward(self, union_feats, det_result, union_rois):
        ...
        if self.bidirectional:
            rel_rank_rep = torch.mean(torch.stack((rel_rank_rep[:, :self.hidden_dim], rel_rank_rep[:, self.hidden_dim:])), 0)
...
class LinearRanker(nn.Module):
    def __init__(self, input_dim=1024, hidden_dim=512, nl_layer=1, num_out=1):
        ...
        for i in range(self.nl_layer):
            ...
            ranking_net += [nn.Linear(dim, self.hidden_dim), nn.ReLU(inplace=True)]
...
def get_size_maps(size, boxes_int, form='rect'):
    h, w = size
    #boxes_int = bbox.long()
    boxes_w, boxes_h = boxes_int[:, 2] - boxes_int[:, 0] + 1, boxes_int[:, 3] - boxes_int[:, 1] + 1
...
def get_weak_key_rel_labels(det_result, gt_result, comb_factor=0.5, area_form='rect'):
    gt_bboxes = gt_result.bboxes
    det_bboxes = det_result.bboxes
    saliency_maps = det_result.saliency_maps
    key_rel_labels = []
    rel_pair_idxes = det_result.rel_pair_idxes
    for rel_pair_idx, gt_bbox, det_bbox, saliency_map in zip(rel_pair_idxes, gt_bboxes, det_bboxes, saliency_maps):
        assert det_bbox.shape[0] == gt_bbox.shape[0]
        det_bbox_int = det_bbox.clone()
        det_bbox_int = det_bbox_int.long()
        h, w = saliency_map.shape[1:]
        det_bbox_int[:, 0::2] = torch.clamp(det_bbox_int[:, 0::2], min=0, max=w - 1)
        det_bbox_int[:, 1::2] = torch.clamp(det_bbox_int[:, 1::2], min=0, max=h - 1)
        object_saliency = torch.cat(
            [torch.mean(saliency_map[0, box[1]:box[3] + 1, box[0]:box[2] + 1])[None] for box in det_bbox_int], 0).float().to(
            det_bbox_int.device)
        object_area = get_size_maps(saliency_map.shape[1:], det_bbox_int, area_form)
        object_importance = object_saliency * comb_factor + (1.0 - comb_factor) * object_area
        pair_importance = object_importance[rel_pair_idx[:, 0]] + object_importance[rel_pair_idx[:, 1]]
