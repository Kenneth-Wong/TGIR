...
class PointNetFeat(nn.Module):
    def __init__(self, init_c=32, out_c=1024, global_feat=True, feature_transform=False):
...
class PointNetDenseCls(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x / torch.max(torch.sqrt(torch.sum(x ** 2, dim=1, keepdim=True)), dim=-1, keepdim=True)[0]
        batchsize = x.size()[0]
        n_pts = x.size()[2]
        x, trans, trans_feat = self.feat(x)
        x = F.relu(self.bn1(self.conv1(x)))
