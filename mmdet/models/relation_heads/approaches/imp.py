...
class IMPContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes):
        ...
        self.edge_gru = nn.GRUCell(input_size=self.hidden_dim, hidden_size=self.hidden_dim)
        self.node_gru = nn.GRUCell(input_size=self.hidden_dim, hidden_size=self.hidden_dim)

        self.sub_vert_w_fc = nn.Sequential(nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid())
        self.obj_vert_w_fc = nn.Sequential(nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid())
        self.out_edge_w_fc = nn.Sequential(nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid())
        self.in_edge_w_fc = nn.Sequential(nn.Linear(self.hidden_dim * 2, 1), nn.Sigmoid())

    def init_weights(self):
        for module in [self.sub_vert_w_fc, self.obj_vert_w_fc, self.out_edge_w_fc, self.in_edge_w_fc]:
            for m in module:
                if isinstance(m, nn.Linear):
                    kaiming_init(m, distribution='uniform', a=1)
        for module in [self.rel_fc, self.obj_fc, self.obj_unary, self.edge_unary]:
    ...
    def forward(self, x, union_features, det_result, logger=None):
        ...
        hx_obj = torch.zeros(obj_count, self.hidden_dim, requires_grad=False).to(obj_rep)
        hx_rel = torch.zeros(rel_count, self.hidden_dim, requires_grad=False).to(obj_rep)
