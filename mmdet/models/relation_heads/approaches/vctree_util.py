...
def generate_forest(pair_scores, det_result):
    ...
    for pair_score, label, bbox in zip(pair_scores, labels, bboxes, ):
        ...
        root = ArbitraryTree(root_idx, float(node_scores[root_idx]), int(obj_label[root_idx]), bbox[root_idx],
                             is_root=True)
        ...
        for idx in list(range(num_obj)):
            if idx == root_idx:
                continue
            new_node = ArbitraryTree(idx, float(node_scores[idx]), int(obj_label[idx]), bbox[idx])
...
class ArbitraryTree(object):
    ...
    def search_best_insert(self, score_map, best_score, insert_node, best_depend_node, best_insert_node,
                           ignore_root=True):
...
def get_overlap_info(infostruct):
    ...
    for bbox, img_shape in zip(bboxes, img_shapes):
        ...
        info2 = intersection.sum(1).view(-1, 1) / float(img_shape[0] * img_shape[1])
