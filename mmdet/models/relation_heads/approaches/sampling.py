...
class RelationSampler(object):
    ...
    def prepare_test_pairs(self, det_result):
        ...
        for p in det_result.bboxes:
            ...
            if self.test_overlap:
                cand_matrix = cand_matrix.byte() & bbox_overlaps(p[:, :4], p[:, :4]).gt(0).byte()
    ...
    def gtbox_relsample(self, det_result, gt_result):
        ...
        for img_id, (prp_box, prp_lab, tgt_box, tgt_lab, tgt_rel_matrix, tgt_rel, tgt_keyrel) in enumerate(
                zip(bboxes, labels, gt_bboxes, gt_labels, gt_relmaps, gt_rels, gt_keyrels)):
            ...
            if tgt_keyrel is not None:
                img_keyrel_labels = torch.zeros(tgt_pair_idxs.shape[0]).long().to(tgt_pair_idxs.device)
            ...
            if img_keyrel_labels is not None:
                img_keyrel_labels = torch.cat((img_keyrel_labels.long(),
                                               torch.ones(tgt_bg_idxs.shape[0], device=device).long()*-1),
                                               dim=0).contiguous().view(-1)
    ...
    def detect_relsample(self, det_result, gt_result):
        ...
        for img_id, (prp_box, prp_lab, tgt_box, tgt_lab, tgt_rel_matrix, tgt_rel, tgt_keyrel) in enumerate(
            ...
            is_match = (tgt_lab[:, None] == prp_lab[None]) & (ious > self.pos_iou_thr)  # [tgt, prp]
            # Proposal self IoU to filter non-overlap
            prp_self_iou = bbox_overlaps(prp_box[:, :4], prp_box[:, :4])  # [prp, prp]
            ...
            img_rel_triplets, binary_rel = sampling_function(device, tgt_rel_matrix, tgt_rel, tgt_keyrel, ious,
                                                             is_match, rel_possibility)
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, tgt_rel, tgt_keyrel, ious, is_match, rel_possibility):
        """
        ...
        for i in range(num_tgt_rels):
            ...
            tgt_key_rel_lab = int(img_keyrel_labels[i]) if img_keyrel_labels is not None else None
            ...
            if tgt_key_rel_lab is not None:
                fg_key_labels = torch.tensor([tgt_key_rel_lab] * prp_tail_idxs.shape[0], dtype=torch.int64,
        ...
        if len(fg_rel_triplets) == 0:
            ...
            fg_rel_triplets = torch.zeros((0, col), dtype=torch.int64, device=device)
        else:
        ...
        if self.key_sample:
            bg_key_labels = torch.tensor(bg_rel_inds.shape[0], dtype=torch.int64, device=device).fill_(-1).view(-1, 1)
            bg_rel_triplets = torch.cat((bg_rel_triplets, bg_key_labels), dim=-1)

        num_neg_per_img = min(self.num_rel_per_image - fg_rel_triplets.shape[0], bg_rel_triplets.shape[0])
        if bg_rel_triplets.shape[0] > 0:
            ...
            bg_rel_triplets = torch.zeros((0, 4 if self.key_sample else 3), dtype=torch.int64, device=device)

        # if both fg and bg is none
        if fg_rel_triplets.shape[0] == 0 and bg_rel_triplets.shape[0] == 0:
            col = 4 if self.key_sample else 3
            bg_rel_triplets = torch.zeros((1, col), dtype=torch.int64, device=device)
            if col == 4:
