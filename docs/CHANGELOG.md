## MMSceneGraph Change Log

### (Self-Adaptive Version) v1.6.0 
- Now we support the scene graph visualization.

- For the VisualGenomeDataset, the groundtruth for test/val set is modified so that the duplicated 
  triplets are filtered. The impact to the recall is ignorable.


### (Self-Adaptive Version) v1.5.0 
- Adjust the feature extraction API for scene graph generation. We add the `frontend_features` function to the `relation_head` and delete all the feature extraction process from different sub-class heads.


### (Self-Adaptive Version) v1.4.0 
**Hightlights**
- Modify the APIs for detection result visualization: Add the patches package under mmdet. It uses colors from seaborn. 
  **Note: The opencv2 "color" must be a triplet tuple containing "int" type.**
- Reimplement the interfaces for supporting `repPoints` extraction and visualization under with the `RepPointDetector`.

### (Self-Adaptive Version) v1.3.0
**Hightlights**
- For supporting the transfer learning of instance segmentation ([Mask-X-RCNN](https://openaccess.thecvf.com/content_cvpr_2018/papers/Hu_Learning_to_Segment_CVPR_2018_paper.pdf)), the following features are added: 
  - Add the `CocoRemapDataset`, which maps 53 COCO categories to the VG 150 categories.
  - Add the `TransferMaskHead`.
  - Add the `ExtrDetWeightSharedFCBBoxHead` to extract the detection weights.
- Add the [shape aware RoI Align](https://arxiv.org/abs/1911.11263) operation.
- Adapt the commition of class wise evaluation result for COCO from the mmdetection github v2.0.
- Adapt the commition of merging dist and non-dist tranining from v2.0.
- Adapt the commition of modification of `gpu_ids` from v2.0.



### (Self-Adaptive Version) vA1.2.0 (06/4/2020)

**Highlights**
- Add the VisualGenome dataset. The data loading and formatting module is modified. 
- Create 'filter_cocoval_from_train_split' in VG-SGG-with-attr.h5 file. It filters 
  the images in VG training split which are also in cocoval2017 split. This split way
  is used for the first stage training of [SegmentEverything CVPR 2018].



##  Original mmdetection Change Log
