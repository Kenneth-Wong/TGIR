...
def preprocess_attributes(data, alias_dict={}):
    for img in data:
        for attr in img['attributes']:
            if 'attributes' not in attr:
                ...
                new_attributes = []
                attributes = attr['attributes']  # list
                for a in attributes:
                    a = sentence_preprocess(a)
                    if a in alias_dict:
                        a = alias_dict[a]
                    new_attributes.append(a)
                new_attributes = list(set(new_attributes))
                attr['attributes'] = new_attributes
...
def encode_objects(obj_data, token_counter):
    ...
    pbar = mmcv.ProgressBar(len(obj_data))
    for i, img in enumerate(obj_data):
        img['id_to_idx'] = {}  # object id to region idx
        for obj in img['objects']:
            # pick a label for the object
            if 'synsets' in obj and len(obj['synsets']) > 0:
                obj_label = obj['synsets'][0].split('.')[0]
            else:
                max_occur = 0
                obj_label = None
                for name in obj['names']:
...
def encode_relationships(rel_data, obj_data):
    ...
    print('Encode relationships...')
    pbar = mmcv.ProgressBar(len(rel_data))
...
def encode_attributes(attr_data, obj_data):
    obj_filtered = 0
    print('Encode attributes...')
    pbar = mmcv.ProgressBar(len(attr_data))
    for i, img in enumerate(attr_data):
        id_to_idx = obj_data[i]['id_to_idx']  # object id to object list idx
        new_attributes = []
        for attribute in img['attributes']:
            if attribute['object_id'] not in id_to_idx:
                obj_filtered += 1
                continue
            else:
                new_attributes.append(attribute)
        img['attributes'] = new_attributes
        pbar.update()

    print('%i attr is filtered by object' % obj_filtered)
...
def obj_rel_attr_cross_check(obj_data, rel_data, attr_data, verbose=False):
    """
    ...
    for i in range(num_img):
        ...
        assert (obj_data[i]['image_id'] == attr_data[i]['image_id'])
        objs = obj_data[i]['objects']
        rels = rel_data[i]['relationships']
        attrs = attr_data[i]['attributes']
        ...
        for attr in attrs:
            if attr['object_id'] in ids:
                num_correct_attr += 1
            elif verbose:
                print(str(attr['object_id']) + ' cannot be found in ' + str(i))
            total_attr += 1
    print('cross check: %i/%i relationship are correct; %i/%i attr are correct' % (
        num_correct, total_rel, num_correct_attr, total_attr))


def sync_objects(obj_data, rel_data, attr_data):
    num_img = len(obj_data)
    num_add_by_rel = 0
    num_add_by_attr = 0
    for i in range(num_img):
        assert (obj_data[i]['image_id'] == rel_data[i]['image_id'] == attr_data[i]['image_id'])
        objs = obj_data[i]['objects']
        rels = rel_data[i]['relationships']
        attrs = attr_data[i]['attributes']
        ...
        for rel in rels:
            ...
            if rel['object']['object_id'] not in ids:
                ...
                num_add_by_rel += 1
        for obj in attrs:
            if obj['object_id'] not in ids:
                new_obj = obj.copy()
                new_obj.pop('attributes', None)
                objs.append(new_obj)
                num_add_by_attr += 1
        obj_data[i]['objects'] = objs
    print('Sync Objects: num added by rel: %d,  num added by attr: %d' % (num_add_by_rel, num_add_by_attr))
...
def main(args):
    ...
    attribute_alias_dict = {}
    if len(args.attribute_synset) > 0:
        print('using attribute synset from %s' % (args.attribute_synset))
        attribute_alias_dict = make_alias_dict_from_synset(args.attribute_synset)
    ...
    assert (len(rel_data) == len(obj_data) and (len(attr_data) == len(obj_data)) and len(obj_data) == len(img_data))
    ...
    sync_objects(obj_data, rel_data, attr_data)

    obj_rel_attr_cross_check(obj_data, rel_data, attr_data)
    ...
    preprocess_attributes(attr_data, alias_dict=attribute_alias_dict)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attribute_input', default=attributes_file, type=str)
    parser.add_argument('--attribute_synset', default=attribute_synset_file, type=str)
